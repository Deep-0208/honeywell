...
function fetchWithRetry(urlPath, retries = 3) {
    ...
    function attempt(n) {
      ...
      req.on('error', (err) => {
        if (n > 1) {
          ...
          resolve({ status: 500, error: err.message, html: '' });
        }
      });
      req.setTimeout(15000, () => {
        ...
        if (n > 1) {
          ...
          resolve({ status: 504, error: 'Timeout', html: '' });
        }
      });
    }
    attempt(retries);
...
async function run() {
  console.log(`Verifying ${pages.length} production routes...`);
  const report = [];

  for (let i = 0; i < pages.length; i++) {
    const p = pages[i];
    const { status, html, error } = await fetchWithRetry(p);
    ...
    console.log(`[${i + 1}/59] ${p} -> Status: ${status} | Canonical: ${canonicalPass ? 'OK' : canonical} | Result: ${allPass ? 'PASS' : 'FAIL'}`);
    // gentle pause between requests to prevent dev server memory throttling
    await new Promise(r => setTimeout(r, 400));
  }

  fs.writeFileSync(path.join(basePath, 'scripts/final_verified_audit.json'), JSON.stringify(report, null, 2), 'utf8');
  console.log(`FINAL RESULT: ${report.filter(r => r.result === 'PASS').length}/${report.length} PASSED.`);
}
