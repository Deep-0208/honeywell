    ...
    >
      ...
      <Container className="relative z-10 flex flex-col gap-12 md:gap-16">
        ...
        <div className="relative flex w-full max-w-7xl mx-auto overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)] group">
          ...
          <div
            className="flex shrink-0 animate-marquee group-hover:[animation-play-state:paused] motion-reduce:animate-none"
            aria-hidden="true"
          >
            {clientLogos.map((client, index) => (
              <div
        ...
        >
          {trustMetrics.map((metric) => (
            <div
              key={metric.label}
              className="flex flex-col items-center justify-center text-center p-3.5 sm:p-4 md:p-6 bg-white rounded-2xl shadow-sm border border-slate-100 last:col-span-2 sm:last:col-span-1"
            >
              <div className="font-display text-[22px] sm:text-[30px] md:text-[40px] leading-tight font-bold text-honeywell-navy mb-1">
                {metric.value}
              </div>
              <p className="text-[10px] sm:text-[11px] md:text-[12px] text-honeywell-red font-display font-bold uppercase tracking-[0.1em] text-center">
                {metric.label}
              </p>
            </div>
          ))}
