    ...
    <>
      {images.map((item, index) => (
        ...
        >
          ...
          <div className="absolute bottom-3 left-3 right-3 sm:bottom-5 sm:left-5 sm:right-5 bg-white/95 backdrop-blur-md border border-slate-200/60 rounded-xl px-4 py-3 sm:px-5 sm:py-4 shadow-sm">
                ...
                {images.map((_, dotIndex) => (
                  <div
                    ...
                    className={`h-1.5 rounded-full transition-all duration-300 ${
                      dotIndex === currentSlide ? 'w-4 bg-honeywell-red' : 'w-1.5 bg-slate-200'
                    }`}
                  />
                ))}
              </div>
